...
int main(void) {
    ...
    int line_length = depth * 2 + 2;
    char *line_string = malloc(sizeof(char) * line_length);


    // have to start at 1 because we use the line number for some calculations
    for (int line = 1; line <= depth; line++) {
        ...
        for (int i = depth + 2; i < line_length; i++) {
            if (i < depth + 2 + line) {
                line_string[i] = '#';
            }
            else {
